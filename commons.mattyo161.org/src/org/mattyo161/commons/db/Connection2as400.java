 ...
 * to an AS400 Server. This funciton should not be called individually
...
class Connection2as400 implements IConnection2Db 
{
    /**
	...
	public String getJdbcUrl(String server,
	        ...
	        String options) 
	{
	    String dbURL = "jdbc:as400://" + server;
        if (!port.equals("")) {
            dbURL += ":" + port;
        }
        dbURL += "/" + db;
     ...
     *AS400 (server, port, user, password, db and options)
     */
    ...
    /**
     *Will load the JDBC driver and create a Connection object for the specified
     *AS400 (server, port, user and password)
     */
    ...
    /**
     *Will load the JDBC driver and create a Connection object for the specified
     *AS400 (server, port, user, password and db)
     */
